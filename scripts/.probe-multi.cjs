...
// electron/acpClient.ts
var import_node_child_process = require("node:child_process");

// electron/mojibake.ts
...
function latin1ishCount(s) {
  let n = 0;
  for (const c of s) {
    const code = c.charCodeAt(0);
    if (code >= 128 && code <= 255) n++;
  }
  return n;
}
var REVERSE_MARKER = /[缂锟拷閿濮栬瘧绛璋]/;
function tryReverseFix(text) {
  if (!REVERSE_MARKER.test(text)) return null;
  let candidate;
  try {
    candidate = import_iconv_lite.default.decode(import_iconv_lite.default.encode(text, "gbk"), "utf8");
  } catch {
    return null;
  }
...
function fixMojibake(text) {
  ...
  if (text.includes(REPLACEMENT)) {
    const candidate = import_iconv_lite.default.decode(Buffer.from(text, "utf8"), "gbk");
  ...
  if (countCjk(text) === 0 && latin1ishCount(text) >= 3) {
    const bytes = Buffer.from([...text].map((c) => c.charCodeAt(0) & 255));
    const candidate = import_iconv_lite.default.decode(bytes, "gbk");
...
var AcpConnection = class {
  constructor(executable, argsPrefix, acpArgs, cwd, cb, envExtra, taskIdKey = "") {
  ...
  turnOnEvent;
  ready = false;
  // handshake+session 建立前为 false：抑制 session/load 的历史回放
  buf = "";
  stdoutNonJson = "";
  // stdout 中非 JSON-RPC 行：kimi 等 CLI 会在此输出 403/认证错误详情
  stderrBuf = "";
  // 累积 stderr：部分 CLI 在 stderr 输出原始 HTTP 错误
  idSeq = 1;
  pending = /* @__PURE__ */ new Map();
  manualClose = false;
  // 主动 kill 时置 true：抑制 onClose 推送 error/system 事件，避免误导用户
  sessionId = null;
  configOptions = [];
  ...
  getDiagnosticOutput() {
    const stdout = this.stdoutNonJson.trim().slice(-1e3);
    const stderr = this.stderrBuf.trim().slice(-1e3);
    return [stdout, stderr].filter(Boolean).join("\n");
  }
  spawn() {
    const proc = (0, import_node_child_process.spawn)(this.executable, [...this.argsPrefix, ...this.acpArgs], {
      cwd: this.cwd,
    ...
    proc.stderr.on("data", (chunk) => {
      ...
      if (match) {
        ...
        for (const [id, p] of this.pending) {
          p.reject(new Error(errMsg.slice(0, 1e3)));
          this.pending.delete(id);
        }
      }
    });
    proc.on("close", () => {
      const stdoutTail = this.stdoutNonJson.trim().slice(-1e3);
      const stderrTail = this.stderrBuf.trim().slice(-1e3);
      const reason = stdoutTail || stderrTail || void 0;
  ...
  onMessage(obj) {
    if (obj.id !== void 0 && (obj.result !== void 0 || obj.error !== void 0)) {
...
${stderrTail}` : "";
    ...
    if (obj.method === "session/request_permission" && obj.id !== void 0) {
      this.handlePermissionRequest(obj.id, obj.params);
      return;
    }
    if (obj.method && obj.id !== void 0 && obj.result === void 0) {
      this.writeRaw({
        jsonrpc: "2.0",
        id: obj.id,
  ...
  handlePermissionRequest(rpcId, params) {
    ...
    const payload = {
      ...
      toolName: String(toolCall.title ?? "tool"),
      summary,
      options
    };
  ...
  resolvePermission(requestId, rpcId, optionId) {
  ...
  async newSession(loadSessionId) {
    if (loadSessionId) {
      try {
        const result2 = await this.request("session/load", {
          sessionId: loadSessionId,
          cwd: this.cwd,
          mcpServers: []
        });
        this.sessionId = result2.sessionId ?? loadSessionId;
        this.configOptions = result2.configOptions ?? [];
...
function mapAcpUpdate(update) {
  ...
  if (type === "tool_call" || type === "tool_call_update") {
    ...
    if (status === "completed" || status === "failed") {
      events.push({
        type: "tool_result",
        toolId,
        result: fixMojibake(contentText).slice(0, 4e3),
  ...
  if (statusStr === "error" || statusStr === "failed") {
    const message = String(
      update.message ?? update.error ?? update.text ?? update.detail ?? update.content?.text ?? `ACP ${type} ${statusStr}`
    );
    events.push({ type: "error", message });
    return events;
  }
...
var AcpManager = class {
  ...
  async ensure(taskId, cli, executable, cwd, envExtra, storedSessionId, modelEntryId) {
        ...
        onClose: (reason) => {
          ...
          if (reason) {
            safeSend(this.sender, "task:event", {
              type: "error",
              message: `ACP \u8FDE\u63A5\u5173\u95ED\uFF1A${reason.slice(0, 1e3)}`,
              taskId
            });
          } else {
    ...
    conn.markReady();
    conn.modelEntrySnapshot = modelEntryId;
    return conn;
  }
  // 发送一条消息（流式事件经 onEvent + sender 推送）
  async prompt(taskId, cli, executable, cwd, text, opts) {
    ...
    conn.setTurnOnEvent(opts.onEvent);
  ...
  async stop(taskId) {
    await this.conns.get(taskId)?.cancel();
  }
  // 对已建立的长驻连接实时设置 config option（如权限模式 mode），无连接时静默跳过
  async setSessionOption(taskId, configId, value) {
...
function makeTextAdapter(buildArgs) {
  return {
    buildArgs,
    parseLine: (line) => {
...
var opencodeAdapter = makeTextAdapter((message) => ["run", message]);
var aiderAdapter = makeTextAdapter((message) => ["--message", message, "--yes-always"]);
var hermesAdapter = makeTextAdapter((message) => ["run", message]);
...
function toSpawnTarget(resolvedPath) {
...
// scripts/probe-multi.ts
var manager = new AcpManager();
async function turn(n, text) {
  const events = [];
  const t0 = Date.now();
  const result = await manager.prompt(
    ...
    {
      onEvent: (ev) => {
        if (ev.type === "delta") events.push(`delta(${ev.text.length})`);
        else if (ev.type === "thinking") events.push(`think(${ev.text.length})`);
        else if (ev.type === "tool_call") events.push(`tool:${ev.name}`);
        else if (ev.type === "error") events.push(`ERROR:${ev.message.slice(0, 80)}`);
        else if (ev.type !== "session") events.push(ev.type);
      }
    }
  );
  console.log(`turn${n}: ${((Date.now() - t0) / 1e3).toFixed(1)}s events=${events.length} session=${result.sessionId?.slice(-6)}`);
  const deltaTotal = events.filter((e) => e.startsWith("delta")).length;
  const errors = events.filter((e) => e.startsWith("ERROR"));
  console.log(`  deltas=${deltaTotal} errors=${errors.length}`, errors.slice(0, 2));
  if (events.length === 0) console.log("  *** EMPTY TURN ***");
}
void (async () => {
  try {
    await turn(1, "\u56DE\u590Dok\u4E24\u4E2A\u5B57");
    await turn(2, "\u7EE7\u7EED");
    await turn(3, "\u7EE7\u7EED");
    manager.killAll();
    process.exit(0);
